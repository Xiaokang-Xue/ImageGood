...
ImageGood 支持上传图片或输入文字完成 AI 修图、文生图、智能抠图、图片增强、去杂物、商品图和封面海报生成。图片任务由服务端异步执行：前端通过 `taskId` 轮询状态，任务成功并保存结果后写入历史记录，失败任务不会消耗创作权益。

仓库同时包含账号认证、创作权益与订单、微信和支付宝支付适配、腾讯云 COS、本地存储、运营后台及任务巡检等产品化基础设施。真实图片、短信、邮件和支付能力需要对应服务凭据；本地可使用 mock 模式启动。

## 为什么选择 ImageGood

| 特性 | 设计重点 |
| --- | --- |
| 完整业务闭环 | 账号、图片任务、创作权益、支付、对象存储、历史记录和运营后台位于同一套应用中。 |
| 可靠任务语义 | 异步状态轮询；结果保存成功后再结算权益；失败不消耗权益；支付回调采用幂等生效逻辑。 |
...
任务创建后立即返回 `taskId`；模型、存储或数据库提交任一环节失败，任务进入 `failed` 且不消耗创作权益。
