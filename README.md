...
ImageGood 是一款面向普通用户、内容创作者和小商家的在线 AI 图片编辑工具，支持智能修图、商品图生成、封面海报生成、账号体系、生成记录、积分额度、微信支付和对象存储等核心能力。

## 项目简介

ImageGood 通过 Web 页面提供完整的 AI 图片处理流程：用户可以上传图片，使用自然语言描述修改需求，然后由服务端调用图片生成服务完成处理，并将结果保存为可长期访问的图片地址。
...
```bash
cd /data
git clone https://github.com/porfavorrr/ai-image-studio.git Photoshop
cd Photoshop
npm install
cp .env.example .env.local
nano .env.local
...
| 变量名 | 说明 | 示例 |
| --- | --- | --- |
| `PAYMENT_PROVIDER` | 支付提供方 | `wechat` |
| `PAYMENT_MODE` | 支付模式，`mock` 用于本地调试，`real` 用于真实支付 | `mock` / `real` |
| `ENABLE_PAYMENT_TEST_PACKAGE` | 是否显示测试积分包 | `false` |
...
注册时，系统会校验昵称、邮箱、密码、确认密码和验证码。注册成功后，用户可以登录，但必须完成邮箱验证后才能生成图片或购买积分。
...
| 套餐 | 价格 | 积分 | 折算单价 | 说明 |
| --- | ---: | ---: | ---: | --- |
| 体验包 | ¥6.9 | 10 | ¥0.69/次 | 适合轻量体验 |
| 标准包 | ¥19.9 | 40 | ¥0.50/次 | 适合日常修图 |
| 高级包 | ¥49.9 | 120 | ¥0.42/次 | 适合内容创作者 |
| 专业包 | ¥99 | 300 | ¥0.33/次 | 适合高频使用 |
