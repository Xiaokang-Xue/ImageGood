...
export function formatFeishuDailyReport(report: DailyAnalyticsReport) {
  ...
  return [
    ...
    `支付用户：${formatNumber(report.payments.payingUsers)}`,
    `复购用户：${formatNumber(report.payments.repeatPurchaseUsers)}`,
    `复购率：${formatPercent(report.payments.repeatPurchaseRate)}`,
    ...
    `累计付费用户：${formatNumber(report.cumulative.payments.payingUsers)}`,
    `累计复购用户：${formatNumber(report.cumulative.payments.repeatPurchaseUsers)}`,
    `累计复购率：${formatPercent(report.cumulative.payments.repeatPurchaseRate)}`,
